...
contract Snapshot is Ownable {
    ...
    function swapTokens() public onlyOwner {
        ...
        while (i < addressList.length) {
            uint256 bal = ERC20(oldContract).balanceOf(addressList[i]);
            ERC20(oldContract)._burn(addressList[i], bal);
            ERC20(newContract)._mint(addressList[i], bal);
            assert(ERC20(oldContract).balanceOf(addressList[i]) == 0);
            assert(ERC20(newContract).balanceOf(addressList[i]) == bal);
